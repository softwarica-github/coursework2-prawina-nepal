...
`""""""`'''
    ...
    def main(self,root):
        ...
        title = Label(f,text='Image Steganography Tool')
        title.config(font=('courier',33, 'bold'), foreground='Green')
        title.grid(pady=10)

        b_encode = Button(f,text="Encode",command= lambda :self.frame1_encode(f), padx=14)
        b_encode.config(font=('courier',14),foreground='Green' )
        b_decode = Button(f, text="Decode",padx=14,command=lambda :self.frame1_decode(f))
        b_decode.config(font=('courier',14), foreground='green')
        b_decode.grid(pady = 12)

        ascii_art = Label(f,text=self.art)
        # ascii_art.config(font=('MingLiU-ExtB',50))
        ascii_art.config(font=('courier',60, 'bold'), foreground='green')

        ascii_art2 = Label(f,text=self.art2)
        # ascii_art.config(font=('MingLiU-ExtB',50))
        ascii_art2.config(font=('courier',14,'bold'), foreground='green')
